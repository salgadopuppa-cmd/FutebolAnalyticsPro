...
// Function to simulate the end of the game and persist coins (fixed)
window.endGame = function() {
    const area = document.getElementById('gameContentArea');
    if (area) {
        area.innerHTML = `
            <p style="color: #f1c40f; font-weight: bold;">Game Over! Come back anytime to earn more coins!</p>
        `;
        setTimeout(() => {
            area.innerHTML = '<p class="sign-in-prompt logged-in">Choose a new game below:</p>';
        }, 2000);
    }
    saveCoinsToStorage();
    showToast('Game ended. Coins saved.', {ttl: 1800});
};

// Login/Logout Listeners — added with existence check
